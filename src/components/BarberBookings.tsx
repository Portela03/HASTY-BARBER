...
import { bookingService, clearFinalizadosBarbeiro, rescheduleService } from '../services/api';
import { useToast } from '../hooks/useToast';
import Toast from './Toast';
import { formatPhoneBR } from '../utils/phone';
import type { BookingResponse } from '../types';

const BarberBookings: React.FC = () => {
  const { id } = useParams();
  const barberId = Number(id || 0);
  ...
  const loadBookings = async () => {
    setIsLoading(true);
    try {
      const data = await bookingService.listByBarber(barberId);
      setBookings(data || []);
      try {
        const first = (data || [])[0] as any;
        const shopId = first?.id_barbearia ?? first?.barbearia?.id_barbearia ?? null;
        if (shopId) {
          const rs = await rescheduleService.listByBarbershop(Number(shopId));
          setReschedules(rs || []);
        } else {
          setReschedules([]);
        }
  ...
  const getStatusBadge = (status: string) => {
    const statusMap: Record<string, { label: string; className: string }> = {
      pendente: { label: 'Pendente', className: 'bg-amber-500 text-gray-900 border-amber-400 shadow-lg shadow-amber-400/20' },
      confirmado: { label: 'Confirmado', className: 'bg-green-600 text-white border-green-500 shadow-lg shadow-green-500/20' },
      em_andamento: { label: 'Em andamento', className: 'bg-purple-600 text-white border-purple-500 shadow-lg shadow-purple-500/20' },
      cancelado: { label: 'Cancelado', className: 'bg-red-600 text-white border-red-500 shadow-lg shadow-red-500/20' },
      finalizado: { label: 'Finalizado', className: 'bg-gray-600 text-white border-gray-500 shadow-lg shadow-gray-500/20' },
    };
    const cfg = statusMap[status] || { label: status, className: 'bg-gray-600 text-white border-gray-500' };
    return (
      <span className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-sm font-semibold border ${cfg.className}`}>
      ...
      <div className="max-w-5xl mx-auto">
        <div className="relative bg-gradient-to-br from-gray-800 via-gray-700 to-gray-800 rounded-2xl shadow-2xl p-8 mb-6 border border-gray-600 overflow-visible">
          <div className="relative z-10">
            ...
            <h1 className="text-5xl leading-tight font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-amber-400 via-yellow-300 to-amber-500 mb-3">Agendamentos do Barbeiro</h1>
            <p className="text-gray-300 text-sm">Visualize e gerencie os agendamentos atribuídos a este barbeiro</p>
          </div>
        </div>
        ...
        <div style={{ animation: 'fadeInUp 420ms ease' }}>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="ml-4">
              <button
                onClick={async () => {
                  if (!confirm('Tem certeza que deseja limpar (marcar como ocultos) os agendamentos finalizados deste barbeiro?')) return;
                  setClearing(true);
                  try {
                    await clearFinalizadosBarbeiro(barberId);
              ...
              >
                <span className="relative z-10">{clearing ? 'Apagando...' : 'Limpar finalizados'}</span>
          ...
          {isLoading ? (
            ...
            <div className="space-y-4">
              {filtered.map((b, idx) => {
                ...
                return (
                  <div key={`${b.id ?? 'bk'}-${idx}`} className="relative rounded-xl p-6 border border-amber-500/30 hover:shadow-2xl transition-all bg-gradient-to-br from-gray-800/60 via-gray-800 to-gray-900">
